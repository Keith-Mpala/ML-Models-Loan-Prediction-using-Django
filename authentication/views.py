...
from django.contrib.auth.models import User, Group  # for registering user in a database
from django.contrib import messages
from django.contrib.auth import login, authenticate, logout
from .forms import RegisterForm, CommentForm, MachineLearning1Form
from django.contrib.auth.decorators import login_required, permission_required # use decorators to redirect us to /login if not logged in : cannot make comments if not logged in
...
@login_required(login_url="/login") #redirect us to login if not logged in already
def home(request):
...
def reviews(request):
    ...
    if request.method == "POST":
        comment_id = request.POST.get("comment_id") # getting the comment_id by clicking e delete btn.
        Comment.objects.filter(id=comment_id).delete()

        user_id = request.POST.get("user_id") # getting user_id to ban user by clicking the btn
...
@permission_required("authentication.add_comment", login_url="/login", raise_exception=True) # redirects to login if not supposed to access comments
@login_required(login_url="/login") # if not logged in, this redirects us to login if we click comment
def create_comment(request):
    if request.method == "POST":
        form = CommentForm(request.POST)
        if form.is_valid():
            comment = form.save(commit=False) # by default commit=T; we do not want to save the form yet cz its incomplete so we add user as well
...
def sign_up(request):
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            return redirect('/login') # after new user has sucessfully signed in, it redirects the user to the sign in page or can be log in page
    else:
        form = RegisterForm()
...
def create_ML1(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            input_variables = [[Loan_amount,Interest_rate,Annual_income,Mort_accounts,Open_accounts,Monthly_debt_income_ratio,Revol_util,Terms,Application_type,Purpose,grade ,Home_ownership,Earliest_cr_line,Public_bankruptcies,Verification_status]]
            feature_names = ['loan_amnt', 'int_rate', 'annual_inc', 'mort_acc', 'open_acc', 'dti', 'revol_util', 'term', 'application_type', 'purpose', 'grade','home_ownership', 'earliest_cr_line', 'pub_rec_bankruptcies', 'verification_status']
            ...
            xg_pred = round(xgboost_model.predict(dtest)[0],2)
            ...
            html_table1 = df.iloc[:,:8].to_html(index=False, header=True, classes="table").replace('<table', '<table style="text-align:center"')
            html_table2 = df.iloc[:,8:].to_html(index=False, header=True, classes="table").replace('<table', '<table style="text-align:center"')
            # Add style to the header row
            html_table1 = html_table1.replace('<th>', '<th style="color:black;">')
            html_table2 = html_table2.replace('<th>', '<th style="color:black;">')
            ...
            context = {'form': form, 'xg_pred':xg_pred, 'rf_pred':rf_pred, 'bg_pred':bg_pred, 'lg_pred':lg_pred, 'html_table1': html_table1, 'html_table2': html_table2}
